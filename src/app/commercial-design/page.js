...
const COMMERCIAL_PROJECTS = [
  ...
  },
  {
    ...
    paragraphs: [
      "微信经营助手作为面向商户的官方智能助手，当前希望通过“支付猫”进一步建立统一的品牌认知，但在会话场景中仍以传统客服形态为主，缺乏智能交互的表达与情绪反馈。本项目围绕智能体验升级与IP动效设计展开，以“支付猫”为核心交互载体，探索其在对话场景中的动态化表达方式，使其从静态视觉符号转向具备状态与情绪反馈能力的智能IP。",
      "在与PM进行沟通后，设计从会话结构入手，梳理不同信息形态在对话中的组织方式，包括文本回复、引导问答、跳转链接、图文卡片及加载状态等，并据此建立基础交互框架，为IP在不同场景中的表达提供稳定的承载逻辑。",
      "在动效实现上，早期尝试过完全手工制作的方式，通过手绘关键帧进行逐帧动画搭建，但在实际执行中发现效率较低，且在高频表情与复杂动作切换的场景下，很难保证运动的流畅性与一致性。基于这一问题，后续转向AI生成与后期精修结合的工作流进行探索与落地：前期通过生成式视频快速验证“支付猫”在不同语境下的动作逻辑与表情变化，以低成本方式完成多轮方案筛选与方向收敛；随后将生成结果拆解为序列帧导入 AE 进行二次编辑，对关键内容（如logo颜色会发生轻微变化，角色不一致等等）进行逐帧修正与节奏重构，重点处理表情稳定性、动作衔接流畅度以及品牌视觉一致性等问题，在生成效率与可控精度之间不断调整平衡，最终形成一套可复用的IP动效生产流程。"
    ],
